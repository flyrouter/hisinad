...
int main(int argc, char** argv)
{
    ...
    int r = HI_MPI_SYS_MmzAlloc_Cached(&phy_addr, &ptr, "usr-tester", 0, SZ_ARRAY * sizeof(unsigned));
    ...
    r = HI_MPI_SYS_MmzFlushCache(phy_addr, ptr, SZ_ARRAY * sizeof(unsigned));
    //r = HI_MPI_SYS_MmzFlushCache(0, 0, 0);
    log_info("MMZ FLUSH returned 0x%x (0 is OK)", r);

    do_test(ptr);

    r = HI_MPI_SYS_MmzFree(phy_addr, ptr);
